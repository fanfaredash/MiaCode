...
Esc 另外需要一个明确的**归属链**（当前只有补全弹窗认领它）：
补全弹窗 → 查找栏 → 收起选区 → 什么都不做。**永远不进正文。**
...
- A1：保存 = 存整份文件；放弃 = 只回滚这个 section。（推荐：与"文件是持久化单位"一致，
  且"放弃"最符合"我不要这个标签的改动"的直觉）
...
- B2：欢迎页 / 空状态，没有可编辑的正文（v1 的 `switchToWelcomePage`）。

**C. 每视图撤销历史在「打开别的文件」后是否保留？**

- C1：不保留，全清。（推荐，也最不容易出事）
- C2：按文件路径保留，切回同一文件时恢复。（内存与陈旧风险都更高）
