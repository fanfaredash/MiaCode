...
## 0. 帧率实测复核（2026-08-05 追加，优先于下文全部排序）

**本报告的前提没有被实测支持。在现有全部日志中，预览播放期帧率稳定命中目标刷新率，没有回退。**

### 0.1 测量方法

本报告原文是纯静态审计，「没有为本报告新增运行时性能采样」。补测使用了工作区已有的六份日志（`logs` … `logs 6`，2026-08-04/05，三台机器）。

`miacode_preview_profile_summary.txt` 的汇总字段不能直接用（原因见 0.3），因此改从运行时日志重建每个播放段的真实帧率：

- 播放段边界：`preview/interaction action=play_complete txn=N` → `preview/resource_gauge reason=pause_exact txn=N`
- 预览呈现数：同一行的 `present_total`（来自 `PreviewRuntime::resourceGaugePayload()`，即 `presentedFrameCountTotal_`）逐段做差
- 帧率 = 呈现数差 / 段时长

⚠️ 同一行里的 `presents_in_play` **不是**预览的呈现数，它是 `leak_gauge::timelinePresentsSincePlayStart()`，即 `TimelineQuickItem::updatePaintNode()` 的调用次数——时间轴的帧率。复核过程中一度误用，得出「帧率被钉在 164」的错误结论。任何人重做此项测量时请勿混淆这两个字段。

### 0.2 结果

三份带 PV 的 180 Hz 会话，36 个播放段，合计 1655 秒：

| 会话 | 机器 | 目标 | 播放总时长 | 实测预览 FPS |
|---|---|---|---|---|
| logs 3 | RTX 4070 Laptop | 180 Hz | 969 s | **180.5** |
| logs 4 | RTX 4070 Laptop | 180 Hz | 573 s | **181.2** |
| logs 5 | RTX 2080 Ti | 180 Hz | 114 s | 177.6 |

所有时长超过 30 秒的段单独看都在 179.7–182.8 之间。logs 5 偏低的唯一原因是它最后一段（11.5 s）里用户最小化了窗口 2153 ms（`occluded_for_ms=2153`），扣除后同样回到目标值。时长不足 3 秒的段会算出 190–380 的虚高值，那是段边界时间戳精度不足导致的，不具参考意义。

另外两份无 PV 的 60 Hz 会话（`logs`、`logs 2`，MX450 笔记本）同样达标：`playback_present_fps` 分别为 59.98 与 60.01。

**结论：180 Hz 恰恰是本报告第 4.1 节预言「回退最明显」的条件，而实测在该条件下精确命中目标。**

### 0.3 汇总文件本身有三处会误导读者

这解释了「帧率很低」这一印象是怎么产生的——三处都指向同一个方向：

1. **顶层 `fps` 字段把空闲时间平均了进去。** 应用闲置时 QSG 本就不呈现，这不是掉帧。三份 PV 会话的顶层 `fps` 分别是 168.4 / 172.7 / 164.3，而实测播放期是 180.5 / 181.2 / 177.6。无 PV 会话更极端：`logs` 的顶层 `fps=0.7184`，同一会话播放期是 59.98。
2. **`playback_*` 整块只覆盖最后一个播放段。** `PreviewRuntime::setActivePlaybackProfilingEnabled(true)` 在每次开启时清零播放期累加器，所以 1655 秒播放里只有最后几十秒进了汇总——而文件头写的是 `profile_scope=session_accumulated`。
3. **`presents_in_play` 字段名极易被读成预览呈现数**，实际是时间轴的（见 0.1）。

### 0.4 唯一仍成立的实测信号

平均值达标，但存在偶发的单帧长间隔：`present_window_max_ms` 为 45.8 / 31.9 / 71.3 ms，在 5.56 ms 的 vsync 下相当于连续错过 6–13 次呈现；`fixed_gate.missed_target_slots_total` 约占呈现总数的 0.6%–1.1%。

这是「偶发微卡顿」，不是「帧率偏低」，两者的排查方向完全不同。现有日志无法定位其成因——这几次会话都没有启用帧节奏诊断（全日志无 `render_frame_profile` 行），因此没有逐帧归因。要继续追这条，需要带 `MIACODE_PREVIEW_FRAME_PACING_DIAG` 重新采样。

### 0.5 对下文排序的影响

第 3 节的可疑度总表与第 9 节的最终判断，是在没有任何帧率测量的前提下按「代码路径、频率和资源规模」推断出来的。现在有了 1655 秒实测，那套排序的前提不再成立：

- 第 4.1 节被列为「当前最强的全局静态候选」，其核心论据是「刷新率越高越明显」。180 Hz 实测精确达标，**该论据被直接证伪**，详见 4.1 的审查回执。
- 第 4.2 节的双适配器结构在三台机器上均未触发（见该节）。
- 第 4.3 节的 180 ms keyed-mutex 等待在约五万帧中触发 **0** 次（`acq_timeout=0`，见该节）。

**建议：在拿到一份能复现帧率不达标的日志之前，不要按第 3 节的排序投入优化。** 优先该做的是修掉 0.3 里那三处误导性指标——否则下一次「帧率很低」的判断仍会建立在同样的错误读数上。
...
> ⚠️ 本表的排序产生于纯静态推断阶段，其前提（预览帧率存在回退）未被实测支持。第 1、2、3 位在实测中分别被证伪或判定未触发，详见第 0 节。请勿按本表排序投入优化。
...
>
> **补充（2026-08-05，实测到手后）：改判为「暂不投入」。**
>
> 本节的核心论据是「刷新率越高越明显」——高刷下帧预算更短，每 tick 4–5 次宽结构拷贝会先撑爆。第 0 节的实测正好覆盖了这个条件：三份 180 Hz 会话、36 个播放段、1655 秒，预览帧率稳定在 180.5 / 181.2 / 177.6，所有长段都在 179.7–182.8 之间。
>
> 换算一下这个条件有多严苛：180 Hz 下每 tick 5 次发布 ≈ 每秒 900 次完整 `PreviewFrameState` 复制，每帧预算只有 5.56 ms。如果这套复制真的在侵蚀预算，180 Hz 就是它必然现形的地方。它没有现形。
>
> 这不推翻本节的代码事实（发布次数、字段宽度、频率全部核实无误），推翻的是「它正在造成可观测的帧率回退」这个推论。宽结构快照仍然是一笔真实的架构债，但它现在没有性能证据支撑，不该排在优化队列前面。
>
> 重新投入的触发条件：拿到一份播放期帧率确实不达标的日志，且该会话的 `playback_*` 指标覆盖足够长的连续播放（注意第 0.3 节说的累加器清零问题）。在那之前，路线 (a) 和 (b) 都不必动。
...
> 审查（2026-08-05）：**代码事实成立，但本节盯错了成本项——180 ms 等待实测触发 0 次，真正每帧都在发生的是纹理分配，本节完全没量化。**
>
> 三点事实全部核对无误：`videoDecodePrefersSoftware_` 默认 `false`（硬解）、H2 单设备默认关闭、`kPreviewAcquireSyncTimeoutMs = 180`（`qavhwdevice_d3d11.cpp:82`）。
>
> **实测：`acq_timeout=0`。** 三份带 PV 的会话（`logs 3`/`logs 4`/`logs 5`，共约五万帧过桥）的每一条 `preview/hwdecode_summary` 都是 `acq_timeout=0 copy_fail=0 frames_decode_error=0 corrupt_dropped=0`，`path=two-device` 确认桥在跑。这条 180 ms 长尾从未发生，不能作为帧率解释。
>
> **但同一批日志暴露了本节漏掉的每帧开销：每条 summary 里 `tex_created` 恰好是 `copied_two` 的 2 倍**（例如 `copied_two=18433 tex_created=36866`）。查 `qavhwdevice_d3d11.cpp` 确认，两设备路径下每个解码帧都要：
>
> 1. `CreateTexture2D` 建 srcTex（`SHARED_KEYEDMUTEX | SHARED_NTHANDLE`）
> 2. 创建共享 NT 句柄，在渲染设备上 `OpenSharedResource1` 导入
> 3. `CreateTexture2D` 建 outputTex
> 4. 两次 `AcquireSync`/`ReleaseSync` + 两次 `CopySubresourceRegion`
>
> 代码注释写得很直白：「this whole per-frame `QAVVideoFrame_D3D11` (srcTex/sharedHandle/destTex + keyed mutex) is rebuilt fresh next frame」。对照之下 H2 单设备路径的 `copyTextureSameDevice()` 只建 1 张纹理、无句柄、无 keyed mutex。
>
> 定性：这是**一直在发生**的每帧驱动分配，比本节原本强调的「可能发生的 180 ms 等待」确定得多。但它同样没有造成可观测的帧率损失——同一批会话的预览帧率稳定在 180（第 0 节）。所以它是一条明确的效率问题，不是当前的帧率问题。若将来要优化视频路径，这里而不是超时值才是着手点。
>
...
> **本节已被第 0 节的实测复核取代。** 下文是静态审计阶段的结论，写于没有任何帧率测量的时候；其核心判断（4.1 为首要回退源）已被 180 Hz 实测证伪。保留原文以便对照推断与实测的差距，阅读时请以第 0 节为准。
...
> 本节的判定完成于实测到手之前。第 0 节的帧率复核之后，4.1 由「取舍」改判为「暂不投入」、4.2 由「取舍」改判为「待触发」、4.3 的重点由超时值转到每帧纹理分配，下表已同步。其余条目的判定不受影响——它们本来就不是按帧率证据排的。
...
| 条目 | 判定 | 说明 |
|---|---|---|
| 4.1 帧状态快照放大 | **暂不投入** | 计数全部核实，但「刷新率越高越明显」被 180 Hz 实测证伪（第 0 节）。仍是架构债，无性能证据 |
| 4.2 高性能 GPU 默认绑定 | **待触发** | 过期注释已改写；但实测日志与代码复核都表明双适配器结构当前不会发生，见该节。默认值无需改动 |
| 4.3 PV 默认硬解 + 两设备桥 | **盯错成本项** | 180 ms 等待实测触发 0 次；真正每帧发生的是 2 次 D3D11 纹理分配 + 共享句柄导入，本节未量化 |
