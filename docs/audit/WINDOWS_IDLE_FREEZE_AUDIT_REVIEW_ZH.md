...
### P-3 日志取证方案不可靠（实测已证实，且比预估严重）

> **2026-08-04 实测**：用户日志 **39 分钟产出 18,033 行 / 4.2 MB（≈ 6.5 MB/h），轮转已经发生**（`miacode_runtime_debug.1.log` 即被顶下的那段）。
>
> 只保 3 段 × 4 MB ≈ 12 MB，按此速率**约 2 小时即把最早证据顶掉**。本文原先建议的 2 小时静置长测，会丢失开头。
>
> 量的大头是 runtime 通道的三个 scope（合计 **73.5%**）：`quick_shell/focus` 27.0%（650 B/行，重复输出三份对象描述符）、`edit/extra_selections_apply_perf` 25.4%、`edit/extra_selections_perf` 21.1%（后两者由 `reason=preview_follow` 驱动，单条仅 0.01–0.05 ms，纯粹量大）。音频通道则由 `bass_sfx_drain` 占 48.0%。
>
> **长测前必须先降噪或扩容**，否则取证窗口必然丢失。
...
### O-1（**已排除 —— 见下方实测**）高性能 GPU 绑定默认开启

> **2026-08-04 实测结论：该机型已排除。**
>
> 用户日志中的分诊行：
>
> ```
> startup/gpu_provider surface=quick_shell_root_window action=skip
>   source=qt_default reason=high_perf_equals_default_adapter luid=0x0:0xf856
> ```
>
> 该机（NVIDIA MX450 + Intel Iris Xe）上 **MX450 本身就是 DXGI 默认适配器**，因此 `fromAdapter` 绑定命中了 `gpu_device_provider.cpp:150-160` 的 no-op 短路，**从未发生绑定**。`quick_shell/device` 亦确认根窗口与预览合成面**都在 MX450 上**，不存在本节假设的跨适配器分裂。
>
> 下文的版本收敛分析（默认翻转发生在 `1.1.0-beta`）**在事实层面仍然成立**，`gpu_device_provider.cpp:109` 的陈旧注释也仍是应修的问题；但把它作为本次故障的首要候选**已被证伪**。
>
> 保留本节的原因：它记录了那条**零成本分诊行**如何在一行之内排除一个 P0 候选。这个分诊模式本身值得复用——先要那一行日志，再决定要不要做实验。
...
> **2026-08-04 实测更新**：O-1 已排除（`reason=high_perf_equals_default_adapter`）。同时实测显示无内存/句柄泄漏、无 VRAM 压力、GUI 心跳 41 个样本无缺号，故 O-6 的"渐进劣化"方向亦无证据支持。但该轮测试**最小化仅 84.7 秒**（报告条件是 5–10 分钟且有浏览器负载），属条件不足的阴性结果。

| 排序 | 候选 | 症状匹配 | 版本窗口 | 现成对照手段 |
|---|---|---|---|---|
| ~~1~~ | ~~O-1 高性能 GPU 绑定（双显卡）~~ | **已排除**（该机 MX450 即默认适配器，绑定 no-op） | 默认开始于 `1.1.0-beta` | 分诊行已给出结论 |
| 1 | O-2 显示器休眠 / 会话锁 / TDR 无处理 | 高 | 一直存在 | 复现矩阵 B/C |
