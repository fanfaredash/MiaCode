...
| **F-4** | `clearTimelineAndPreview()` 清了 `muriAnalysisReport_` 但没清 `muriStaticReferences_` | 低（正确性，非泄漏） | 已修复（2026-09-08） | `src/app/runtime/document/` |
...
> ### ⚠️ 实测推翻了 §2.5「影响面」里的性能推断，F-1 严重度据此下调
>
> §2.5 曾推测缓存膨胀会「恶化局部性」。**实测反证**：缓存从 ~18 涨到 ~360 条（20 倍）
> 期间，`updatePaintNode` 平均耗时从 0.334 ms **降到** 0.033 ms —— 热缓存全是命中。
> 995 个采样中 994 个 < 3.4 ms；唯一的 65 ms 离群值是**启动首帧**（`samples=4`，
> 启动后 5.4 s），与缓存规模无关。整个 360 条缓存按 ~3.5 KB/条约合 **1.3 MB**。
>
> ⇒ **F-1 只有常驻内存代价，没有帧时代价。** 严重度从「高」下调为「低」。
>
> 这直接改变了修复的取舍：冲刷本身要拆掉整棵节点树并重传工作集（一次可见卡顿）。
> 一个在正常使用中会触发的上限，等于**拿约 1 MB 换周期性卡顿**——在一个用户正因
> 卡顿抱怨的应用里，这是亏的。因此上限设为 8192 条（约为实测工作集的 20 倍以上，
> 相当于数百次切谱），定位是**失控兜底，不是工作集预算**。
...
> **已修复（2026-09-08）**：QML v2 的 `DocumentSessionHost::clearTimelineAndPreview()`
> 现在与 `muriAnalysisReport_` 同步清空 `muriStaticReferences_`；旧 worker 结果仍由
> revision、难度、文本和 marker signature 校验阻止回填，因此新分析落地前不会再
> 渲染旧难度的静态引用。
