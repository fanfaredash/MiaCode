...
1. 图层列表行拖拽超过列表顶部时，应该判定为拖拽到第一行的位置，而非禁止拖拽。
2. 点击画布空白区域的动作应该是取消图层选择；当前实现是什么都不做。
3. 添加谱面帧的默认行为应该是添加和上一次选中的谱面帧配置完全相同的帧（位置除外）；如果上一次没选中过谱面帧，则复制列表中的第一个谱面帧配置（位置除外）。
4. 当鼠标点击画布两个图层的交叠区域时，应该只触发一个图层。优先级为：正在选中的图层 > 图层顶层 > 次顶层 ... > 图层底层。当前实现会触发多个图层，移动时甚至有崩溃风险。
5. 画布视觉缩放、还原等功能仍需补充；该功能本轮暂缓，不进入当前验收缺口的立即修复范围。
...
测试：扩 `cover_layout_model_spec` 或新增 state spec，验 presets/recent round-trip。
...
### P5 · 列表 delegate 打磨（§7.2 行内，stretch）
