...
### 7.-2 自动保存的**每次编辑安全网**在 v2 从未运行（2026-08-30 所有者报告，已复核，未修）
...
**修法方向**（未执行）：v2 的编辑提交点是 `QmlDocumentModel` 的工作区提交
（`publishWorkspaceCommit`），那里已经拿到了完整文档文本。把 `markCurrentFieldDirty()`
里那两件事——重启 2 秒防抖、投递崩溃快照——挂到那个提交点上即可；
不要去复活隐藏输入框。注意 `currentDocumentTextForAutosave()` 取的是镜像文档，
在 v2 下是当前的，可以直接用。
