...
- **Before trusting `symbol=(nosym)` as "no PDB", check `sym_ready`.** The watchdog thread writes one `action=stack_symbols sym_attempted= sym_ready= sym_err= sym_invaded= sym_invade_err=` line at startup with the outcome of its one-time `SymInitialize`, and each `action=gui_thread_stack` header repeats `sym_ready=`/`sym_invaded=` so a single stack is self-contained. `sym_ready=1` means the symbol handler is up and `(nosym)` really is the normal missing-PDB case; `sym_ready=0` means it never came up and **no** frame in that run could have been named regardless of PDBs — report `sym_err=` rather than filing it as a symbol-less machine. `sym_attempted=0` is the non-Windows build, where there is no dbghelp to initialise.
- `sym_invaded=` says which route brought the handler up. `SymInitialize(fInvadeProcess=TRUE)` enumerates every loaded module as one unit and fails as a unit, so a single uncooperative module denies symbols to the whole process — a field capture came back `sym_ready=0 sym_err=3221225476` (`0xC0000004`, an NTSTATUS left in the last-error slot by an internal query) with every frame of every stack unnamed. It is now retried with `FALSE` plus `SymRefreshModuleList`, so `sym_invaded=0 sym_ready=1` is a **working** handler that took the fallback (`sym_invade_err=` records what the preferred route hit, and is the number to report), while `sym_invaded=1 sym_ready=1` is the ordinary path. With `SYMOPT_DEFERRED_LOADS` set, per-module data loads on demand either way.
...
- `MIACODE_UI_HANG_IDLE_HEARTBEAT_MS` (default `5000`; how long the GUI heartbeat may stop, with no phase marked, before `ui/hang_watchdog` reports. **Lower this when you need a stack for a short stall.** Since `action=gui_thread_stall` landed, a sub-threshold stall is always logged with its duration, so this no longer decides whether a short freeze is *visible* — only whether it gets a hang report and, with it, the Windows GUI-thread stack capture, the one probe that can name the blocking call. A stall that drops ~2 s of preview ticks — an audio output-device switch is the known case — never reaches `5 s`; setting `800` promotes it to a full hang report with a stack.)
