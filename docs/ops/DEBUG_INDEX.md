...
- runtime log:
  ...
  - BASS `bass_sfx_scheduler action=anchor` records a chart/decode-position anchor. `bass_sfx_mixer_trigger group_idx=… group_second=… count=… started_bgm=…` is written once per group the master-mixer sync actually emits — the path a LIVE session uses. Its GUI-fallback counterpart is `bass_sfx_drain`; between the two, every note-SFX emission is accounted for, which is what makes "did an SFX fire after the transport was paused?" answerable from a capture. Written after the scheduler mutex is released, because this runs on the BASS mixer thread.
