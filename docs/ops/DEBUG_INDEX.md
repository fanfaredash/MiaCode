...
- `MIACODE_BASS_MASTER_BUFFER_MS` (Windows/macOS BASS preview master mixer; default `0`, valid range `0..500` ms. Set `0` together with `MIACODE_BASS_MIXER_THREADS=8` to reproduce the former zero-buffer configuration for A/B. Invalid or non-finite values fall back to `0`. The scheduler advances mixer syncs by the effective BASS read-back value so note SFX remain aligned with audible output.)
