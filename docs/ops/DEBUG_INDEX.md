...
- runtime log:
  ...
  - `preview/playback action=pause_second_from_audio wall_second=… audio_second=… stall_ms=…` is written when the device-change auto-pause takes its pause second from the BGM position instead of the wall clock. `stall_ms` is how far the process stalled through the device switch: the wall clock is a `QElapsedTimer` and does not stall with it, so that many milliseconds were never rendered and never heard. Absent when the switch cost nothing, which is the healthy case. Cross-check against `bass_transport op=pause_exact second=…` — after this change the two agree instead of differing by `stall_ms`.
