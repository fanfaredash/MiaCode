...
- runtime log:
  ...
  - `preview_audio_device action=outputs_changed default_changed=…` records Qt audio-output enumeration changes. Only `default_changed=1` can request recovery. `preview/sfx_clock` changes to `source=wall` at a `50 ms` live BGM-clock divergence, and `preview/audio_reanchor` records the requested reason, pre/post deltas, and whether the BASS transport anchor was applied; no row is an end-of-track restart.
  - BASS `bass_transport action=automatic_reanchor` confirms the audio-only transaction that repositioned BGM/SFX to the still-running wall-clock chart second.
