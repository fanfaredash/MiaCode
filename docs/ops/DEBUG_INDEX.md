...
- `ui/hang_watchdog` samples the GUI heartbeat every `250 ms`; it reports a marked phase after about `2 s` or an unmarked idle heartbeat stall after about `5 s` using a durable fatal-grade runtime record. `action=installed` also states whether the GUI-thread stack-walk target was registered.
...
- runtime log:
  ...
  - BASS follows the Windows default output directly; there is deliberately no Qt output-enumeration reset path. `preview/sfx_clock` changes to `source=wall` at a `50 ms` live BGM-clock divergence, and `preview/audio_reanchor` records the requested reason, pre/post deltas, and whether the BGM-only correction was applied; no row is an end-of-track restart.
  - BASS `bass_sfx_scheduler action=anchor` records a chart/decode-position anchor. `bass_transport action=automatic_reanchor scope=bgm_only` confirms the correction seeks only BGM, preserving the master-mixer SFX chain.
