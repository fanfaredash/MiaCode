...
本版本修复导出音效与实时预览不一致的问题。

### 修复

- 修复时间区间相邻或重叠的 touch hold 在导出视频里不重新播放上升音效、而是接着上一个 touch hold 的进度继续播放的问题。导出现在与预览采用同一套音效归属规则，无缝相接、重叠与嵌套三种情况的表现完全一致；从谱面中途开始的部分区间导出，也会像预览跳转那样从采样中段续播。

### 改进

- 开发构建的自动化测试改为在源码目录下运行，避免依赖运行时资源（如音效目录）的用例在 ctest 中误报失败。

## 1.1.0-beta.13

本版本聚焦编辑器交互与 macOS 拖拽体验。

### 修复

- 修复触区创作在编辑器中按播放头位置落点、而非按光标位置落点的问题。
- 修复启用片头后，导出与预览的媒体与谱面跳转不同步的问题。
- 修复 macOS 下拖拽选择划过行号栏时的闪烁问题；元数据编辑器复用同一套拖拽自动滚动逻辑。
...
- 修复预览音频未预加载、以及提交修改后跳转位置丢失的问题。
- 预览混音器改为按项目持久化，不再在切换时被丢弃。
- 修复批量导出页在难度徽标切换后预览未同步的问题。
- 修复仅在 Release 构建下触发的崩溃。

## 1.1.0-beta.11

本版本聚焦背景视频（PV）预览、导出片头与烟花特效。
...
- 修复 PV 追帧期间播放时钟被冻结，以及每帧重复重建预热资源的问题。
- 导出预滚阶段现在会冻结 PV、压平暂停图标，并解除 Enter 键绑定。
- 修复导出片头未应用自定义字体的问题。
- 修复时间轴播放覆盖层的节奏抖动，以及查找浮层的内边距问题。
- 修复模态对话框被 QuickShell 遮挡的问题。
- 修复音轨结束后 BGM 仍在发声、下载未完整的谱面被接受、以及取消下载不及时的问题。
- 退出流程统一走窗口关闭，保证保存提示不会丢失。

## 1.1.0-beta.10

本版本修复 macOS 打包依赖与播放流畅度问题。
...
- macOS 发布包不再依赖 Homebrew，自带 FFmpeg 运行时。
- 修复时间轴滚动量化与采样相位导致的播放抖动。
- 修复触区创作的稳定性问题。
- 关闭调试构建中的看门狗崩溃探针。

## 1.1.0-beta.9

本版本完成预览音频后端的工作线程化、移除 DComp 渲染后端，并补齐 Windows 空闲卡顿与 PV 内存诊断。

### 新增

- 预览音频后端迁移至独立工作线程：新增命令协议、非 GUI 完成屏障与后端持有型 worker。
- 新增 PV 内存诊断记账与专用日志通道，可记录 PV 内存生命周期证据。
- 新增 Windows 空闲卡顿诊断套件：空闲事件日志、进程资源采样、GUI 心跳停顿检测与复现工具。
- 新增拖入音频创建谱面，以及预览「按区域擦除」模式。
- 新增批量上传队列的拖拽排序。
- macOS 新增调试启动器，并提供仓库本地的 FFmpeg SDK。

### 改进

- 移除 DComp / D3D11 渲染后端及其六个调试开关，预览与时间轴统一走 QSG 渲染路径。
- 长时间运行的日志量削减约 70%，重复诊断改为边沿触发。

### 修复

- 音频输出设备变化时立即暂停预览，输出时钟漂移后重新锚定，音效改由 BASS 混音器调度。
- 修复 VideoToolbox 下 PV 纹理生命周期越界的问题。
- 修复切换谱面时资源未及时释放，以及时间轴纹理缓存没有上限的问题。
- 批量上传的文件夹扫描改为自然数排序。
...
- 打磨导出 HUD 的字形排版与 touch 判定时序。

## 1.1.0-beta.7

本版本继续完善封面导出、跨平台体验与预览稳定性，并将扩展能力带入开发构建。

### 新增

- 预览传输栏新增谱面确认 / 无理检测渲染模式切换：谱面确认使用低存在感圆点，无理检测使用问号标识；问号与倍速文本使用相同的主题前景色。
- 封面导出支持自动识别 SD / DX 谱面；片头卡片类型默认可设为“自动”。
- 封面编辑器支持自定义字体，以及图片、文本图层；批量导出设置现可直接嵌入导出页使用。
- 随开发构建附带“宠物悬浮窗”扩展，便于开箱即用地体验扩展系统。

### 改进

- 更新日文界面文本，并支持多个反引号的谱面语法。
- 针对 macOS 补齐 BASS 音频后端、按目标架构瘦身的打包流程，以及嵌入式菜单与 Dock 图标的体验优化。
- 收紧预览传输栏的宽度预算：倍速按钮可容纳 `1.25x`，同时提高预览区最小宽度，避免窄窗口下控件被挤压。

### 修复

- 修复 D3D11 预览对 10-bit 视频格式的保留，以及背景视频结束时意外结束预览传输的问题。
- 修复媒体工具处理 `pv.mp4` 后未确定关闭解码器句柄的问题，并保留压缩后的输出。
- 修复导出 / 元数据页切换后预览时间戳丢失、卡片字体变更后片头覆盖层未刷新、批量导出未保留自动片头模式等问题。
- 修复封面编辑器、字体选择器和导出进度窗口的多处布局与交互问题。

## 1.1.0-beta.6

本版本聚焦可定制的工作区、触区创作与扩展宿主，并补齐 macOS 的 QuickShell 使用体验。

### 新增

- 新增应用工作区背景与遮罩设置，可在 QuickShell 中显示，并针对元数据页等场景保持合适的可读性。
- 新增触区创作快捷开关、触区悬停与判定效果打磨，以及按区域配置 HUD 字体的能力。
- 扩展系统扩展为可用的 v1 闭环：支持扩展开发工具、打开桥接能力与更完整的 API / 权限支持。
- 新增 DX 判定效果样式、判定文本距离设置，以及更多曲目音频格式支持。

### 改进

- 改进编辑器偏好设置持久化、触区创作交互、预览视觉效果和自定义 outline 显示。
- 优化 macOS 打包、原生窗口外壳、全屏生命周期与弹窗定位；新增 VideoToolbox 硬件编码支持。

### 修复

- 修复 macOS QuickShell 原生窗口、全屏、导出流程及完成弹窗的若干显示和生命周期问题。
- 修复编辑器替换撤销后选区恢复、导出倒计时选项持久化，以及仅含控制字符的谱面 token 解析问题。
...
- 修复从 SD 皮肤切换到 DX 皮肤时可能闪退的问题。时间轴 Quick 渲染路径现在会在皮肤、窗口、DPR 或主题变更导致贴图缓存失效前先丢弃旧 QSG 节点树，避免旧节点继续持有已释放的 `QSGTexture`。
- 将应用版本推进到 `1.1.0-beta.4`。

## 1.1.0-beta.3

本版本继续打磨 1.1.0 beta 线，重点是本地扩展系统、多语言收敛、谱面整理、解析诊断、时间轴控制和若干编辑器/导出修复。

### 新增

- 恢复并完善本地扩展系统实现，包含扩展 manifest、内置 JavaScript 运行时、权限/启用管理、偏好设置页、扩展 README、Hello World 模板、TypeScript API 声明和本地校验工具。
- 新增英文 UI 文本表，并恢复/补齐内置日语本地化。
- 新增谱面整理分音段策略，改进非 384 网格、特殊分音段、普通 `||` 注释和尾部 `{N}` carry 的处理。
- 时间轴新增 header 控制与亮度菜单，可调整波形和网格线显示亮度。
- 新增 `skinSD` 判定文字、FAST/LATE、slide judge 等素材。

### 改进

- 将大量 UI 文案迁移到统一 key 路径，移除旧的分散 fallback 文案路径。
- Muri 详情文本改为结构化本地化，连锁 slide 检测更贴近 MaiMuriDX。
- 调整谱面整理弹窗选项，降低隐藏旧偏好影响整理结果的可能。
- 优化编辑器括号补全：选中文本时插入括号会替换为一对空括号，并把光标放入括号内。
- 调整 parser 对 slide 诊断和 mine 大小写的处理。
- 支持打开文件夹用于导出/打开流程，相关提示同步本地化。
- 打磨多处本地化 UI 标签。

### 修复

- 修复 `Ctrl+B` 区间全 Break 的选区解析问题。
- 修复对话框 combo box 样式回退。
- 修复部分补全弹窗点击行为。
- 修复谱面整理在边界场景下的时值与性能问题。
- 修复部分导出、本地化和菜单标签的小问题。

## 1.1.0-beta

本版本从 1.0.1 beta/test 线升级到 1.1.0 beta，重点是预览/导出 GPU 路径、导出设置、时间轴、书签侧边栏和谱面诊断打磨。`1.0.1-beta.3-test`、`1.0.1-beta.4-test`、`1.0.1-beta.5-test` 的内容不单列，合并记录在这里。

### 新增

- 引入 Windows 高性能 GPU 策略骨架、adapter 可见性日志和进程身份日志。
- 新增 D3D11 QRhi 导出后端；OpenGL 导出路径保留为回退与对照。
- 默认启用高性能 GPU 偏好，并补充 GPU/RHI/导出后端诊断。
- 新增 BG/PV 缩放模式，包括 square-fit 与内圈/外圈组合显示模式。
- 新增共享 Skin 设置与布局 hang 诊断。
- 新增 metadata settings 工具菜单入口。
- 重做编辑器书签侧边栏，后续改为基于 `||` 注释的轻量导航模型。

### 改进

- 导出页、导出设置、Gameplay 设置、Skin 设置和批量导出面板使用更统一的控件布局。
- 改进导出页最小宽度，避免 export 页面 workspace 被压得过窄。
- 改进暂停时判定区辅助视图，支持自定义 outline 与区域标签组合显示。
- 时间轴 header marker、缩放步进三角、Quick timeline 控件和亮色主题括号颜色得到打磨。
- Muri 诊断在打开谱面后能更及时刷新。
- 缺失逗号警告更准确地定位到 note 附近。
- 侧边栏难度工具、书签点击导航、书签状态显示和行编辑反馈得到打磨。
- 打开最近文件前增加确认提示，降低未保存内容被覆盖的风险。

### 修复

- 修复导出预览 resize 时的 render snapshot 崩溃。
- 修复嵌入式导出完成状态。
- 修复内圈 BG/PV scale 合成和外圈填充问题。
- 修复时间轴零时长 touch-hold 显示问题。
- 修复时间轴 header marker 重建问题。
- 修复导出 Gameplay/Skin/shared settings 控件刷新问题。
- 修复谱面变换与预览细节相关问题。

## 1.0.1-beta.3

本版本继续打磨 1.0.1 beta 线，重点是谱面变换、预览皮肤资源和片头音效设置。

### 新增

- 新增可配置的片头音效选择。
- 时间轴资源会跟随当前选择的皮肤。
- 工具栏音频相关入口命名调整，使含义更清晰。

### 改进

- 改进谱面变换和预览表现。
- 修复谱面变换后的规范化边界问题。
- 补充 beta 版本号到 `1.0.1-beta.3`。

## 1.0.1-beta.2

本版本是 1.0.1 beta 线的小更新，主要整理皮肤辅助工具和 outline 画布工具。

### 新增

- 简化 outline canvas expansion 工具。
- 将版本号推进到 `1.0.1-beta.2`。

## 1.0.1-beta.1

1.0.0 后的第一个 beta 版本，主要补充本地编辑器书签和皮肤资源整理。

### 新增

- 新增本地编辑器书签能力。
- 标准化皮肤资源目录，加入皮肤工具 helper。
- 新增 `assets/skin`、`assets/music`、`assets/background/outlines` 等资源说明。
- 新增自定义判定线资源与皮肤辅助脚本。

### 变更

- 应用版本从 `1.0.0` 推进到 `1.0.1-beta.1`。
