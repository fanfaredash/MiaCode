...
// One row per mixer channel: its id, its catalog id, the pair of accessors that
...
QVariantList AudioSettingsModel::channels() const
{
    ...
    for (const ChannelSpec& spec : channelSpecs()) {
        rows.append(QVariantMap{
            ...
            {QStringLiteral("label"), qtTrId(spec.labelKey)},
...
void AudioSettingsModel::setChannelPercent(const QString& key, int percent)
{
...
void AudioSettingsModel::toggleChannelMuted(const QString& key)
{
    const ChannelSpec* spec = specForKey(key);
    if (surface() == nullptr || spec == nullptr) {
        return;
...
void AudioSettingsModel::saveAsSoftwareDefault()
{
    if (surface() != nullptr) {
...
void AudioSettingsModel::restoreSoftwareDefault()
{
    if (surface() != nullptr) {
...
void AudioSettingsModel::setAuditionHeld(bool held)
{
...
void AudioSettingsModel::releaseAudition()
{
...
void AudioSettingsModel::queueAudition(const QString& kind)
{
    ...
    if (auditionTimer_ == nullptr) {
        ...
        connect(auditionTimer_, &QTimer::timeout, this, &AudioSettingsModel::flushAudition);
    }
    auditionTimer_->start();
}

void AudioSettingsModel::flushAudition()
{
    if (auditionHeld_) {
...
bool AudioSettingsModel::playAudition(const QString& kind)
{
...
void AudioSettingsModel::setBreakSlideTailCheerMuted(bool muted)
{
