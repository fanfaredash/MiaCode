...
// changes, and preview state lives in PreviewModel rather than in the same
// bag.
//
...
class TimelineModel final : public QObject
{
    ...
    // Localized labels refresh when LocaleService switches language.
    Q_PROPERTY(QString timelineTabLabel READ timelineTabLabel NOTIFY localeLabelsChanged)
    Q_PROPERTY(QString validationTabLabel READ validationTabLabel NOTIFY localeLabelsChanged)
    Q_PROPERTY(QString muriTabLabel READ muriTabLabel NOTIFY localeLabelsChanged)
    Q_PROPERTY(QString followCodeLabel READ followCodeLabel NOTIFY localeLabelsChanged)

public:
    explicit TimelineModel(miacode::ShellNotifications& notifications,
...
private:
    ...
    miacode::TimelineSurface** surfaceSlot_ = nullptr;
    miacode::TimelineSurface* surface() const
    {
