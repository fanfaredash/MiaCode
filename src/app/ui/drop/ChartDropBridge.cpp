...
ChartDropBridge::ChartDropBridge(QObject& window,
...
ChartDropBridge::~ChartDropBridge()
{
    release();
}
...
void ChartDropBridge::setDragState(bool active, const QStringList& paths)
{
...
void ChartDropBridge::scheduleDragLeave()
{
...
void ChartDropBridge::clearDragLeaveTimer()
{
    if (leaveTimer_ != nullptr) {
        leaveTimer_->stop();
    }
...
void ChartDropBridge::submitDrop(const QStringList& paths)
{
    clearDragLeaveTimer();
    setDragState(false);
    if (busy_) {
    ...
    const auto finished = [this, requestId, requestGeneration](const ChartDropResult& result) {
    ...
    if (!submit_) {
        finished(ChartDropResult{requestId, requestGeneration, true, true, false, 0, 1, {}});
        return;
    }
    submit_(paths, requestId, requestGeneration, finished);
}

bool ChartDropBridge::eventFilter(QObject* watched, QEvent* event)
{
