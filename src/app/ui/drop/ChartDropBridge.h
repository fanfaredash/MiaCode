...
class ChartDropBridge final : public QObject
{
    Q_OBJECT
...
public:
    using Submit = std::function<void(const QStringList&, quint64, quint64,
                                      std::function<void(const ChartDropResult&)>)>;
    using Completion = std::function<void(const ChartDropResult&)>;

    ChartDropBridge(QObject& window, std::function<void()> enableDropTarget, Submit submit,
...
signals:
    ...
    void busyChanged();
    void dropBusy();
    void dropCompleted(const ChartDropResult& result);
