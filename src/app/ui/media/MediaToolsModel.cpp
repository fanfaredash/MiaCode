...
MediaToolsModel::MediaToolsModel(                               miacode::UiRequestService& uiRequests,
                                       miacode::JobProgressService& jobProgress,
                                       miacode::MediaToolsEngine*& engineSlot,
                                       QObject* parent)
    : QObject(parent)
...
QVariantList MediaToolsModel::batchJobs() const
{
    QVariantList list;
...
void MediaToolsModel::convertTrackTo44100Hz()
{
    if (engine() != nullptr) {
...
void MediaToolsModel::compressBackgroundVideo()
{
    if (engine() != nullptr) {
...
QVariantMap MediaToolsModel::detectPrependTiming(bool isTrack)
{
    return engine() != nullptr ? engine()->detectMediaBlankTiming(isTrack) : QVariantMap();
}

void MediaToolsModel::restorePrependBackup(bool isTrack)
{
    if (engine() != nullptr) {
...
void MediaToolsModel::applyPrepend(bool isTrack, double beats, double bpm)
{
    if (engine() != nullptr) {
...
void MediaToolsModel::chooseBatchDirectory()
{
...
void MediaToolsModel::setBatchDirectory(const QString& path)
{
...
void MediaToolsModel::addBatchFolder()
{
...
void MediaToolsModel::rescanBatchDirectory()
{
...
void MediaToolsModel::removeBatchJob(int index)
{
...
void MediaToolsModel::clearBatchQueue()
{
    if (batchRunning_) {
        return;
    }
...
void MediaToolsModel::startBatchCompression()
{
    miacode::UiRequestService* const requests = uiRequests_;
    ...
    connect(jobProgress, &miacode::JobProgressService::cancellationRequested, this,
            [this](quint64 token) {
