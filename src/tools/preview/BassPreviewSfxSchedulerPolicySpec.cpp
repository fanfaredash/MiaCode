...
int main()
{
    ...
    ok &= require(
        defaultPolicy.bufferMs == 30.0 && defaultPolicy.threadCount == 1,
        QStringLiteral("the master mixer defaults to a small safety buffer and one mixing thread"), err);
    ...
    ok &= require(
        invalidPolicy.bufferMs == 30.0 && invalidPolicy.threadCount == 1
