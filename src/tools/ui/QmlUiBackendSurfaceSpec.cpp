...
// std::unique_ptr<MainWindow> in Bootstrap. These three are the smart
...
BackendSurface scanQmlUiTree()
{
    ...
    QDirIterator walk(QStringLiteral(MIACODE_SOURCE_ROOT) + QStringLiteral("/src/app/ui"),
...
int main()
{
    ...
    ok &= require(surface.filesScanned > 0,
                  QStringLiteral("the src/app/ui scan found files"));
    ...
    ok &= requireSetEquals(surface.methods + surface.privateMembers, inventory,
                           QStringLiteral("MainWindow surface reached from src/app/ui"));
