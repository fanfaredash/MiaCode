...
miacode::ui::AnalysisProjectionInput alignedInput()
{
    ...
    input.validation.issues = {{3, 4, 4,
        miacode::ui::DocumentValidationIssueSeverity::Error, QStringLiteral("bad note")}};
...
bool verifyWorkspaceSnapshotProjection(QTextStream& err)
{
    ...
    const QVector<miacode::ui::AnalysisRow> muriRows = {
        {8, 2, 2, 12.5, QStringLiteral("error"), QStringLiteral("muri"),
         QStringLiteral("Overlap"), QStringLiteral("two notes overlap")},
    };

    const auto current = miacode::ui::projectAnalysis(snapshot, 5, 72, muriRows);
    ...
    for (const auto& stale : {
             miacode::ui::projectAnalysis(snapshot, 4, 72, muriRows),
             miacode::ui::projectAnalysis(snapshot, 5, 73, muriRows),
             [&] {
                 auto pending = snapshot;
                 pending.available = false;
                 pending.pending = true;
                 pending.noteMarkers.clear();
                 return miacode::ui::projectAnalysis(pending, 5, 72, muriRows);
             }(),
         }) {
...
int main()
{
    ...
    const auto aligned = miacode::ui::projectAnalysis(alignedInput());
    ...
    for (auto mismatch : {[] { auto input = alignedInput(); input.muriSignatureAligned = false; return input; }(),
        ...
        const auto projection = miacode::ui::projectAnalysis(mismatch);
    ...
    const auto noResultProjection = miacode::ui::projectAnalysis(noResult);
    ...
    const auto staleStaticProjection = miacode::ui::projectAnalysis(staleStaticReferences);
    ...
    miacode::ui::AnalysisActivationState activation;
    ...
    activation.begin(currentRow);
    miacode::ui::AnalysisRow completed;
    ok &= require(activation.complete(currentRow, &completed) && !activation.hasPending()
                       ...
                       && miacode::ui::analysisRowCanActivate(aligned, completed, 5),
