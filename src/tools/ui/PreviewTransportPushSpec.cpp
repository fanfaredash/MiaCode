...
int main(int argc, char** argv)
{
    ...
    // 4. QML 侧接收推送，且不再自己采样。
    const QString modelHeader = readSource(QStringLiteral("src/app/ui/preview/PreviewModel.h"));
    const QString modelSource = readSource(QStringLiteral("src/app/ui/preview/PreviewModel.cpp"));
    // The window still raises both, but the model subscribes through the
    // miacode::ShellNotifications relay now — it no longer holds a
    ...
    expect(modelSource.contains(
           ...
           QStringLiteral("PreviewModel listens for the playhead"), out, &failed);
    expect(modelSource.contains(QStringLiteral("ShellNotifications::presentationChanged"))
           ...
           QStringLiteral("PreviewModel listens for shell presentation"), out, &failed);
    ...
    expect(!modelHeader.contains(QStringLiteral("QTimer"))
           ...
           QStringLiteral("PreviewModel polls nothing"), out, &failed);
