...
{
    const QString syncHeader = readSource(QStringLiteral("src/app/services/EditorSyncController.h"));
    const QString syncImpl = readSource(QStringLiteral("src/app/services/EditorSyncController.cpp"));
    const QString sessionHeader =
        readSource(QStringLiteral("src/app/ui/export/ExportSession.h"));
    ...
    const QString seedBody =
        functionBody(sessionImpl, QStringLiteral("ExportSession::seedFromDifficulty"));
    ok &= require(
    ...
    ok &= require(
        sessionHeader.contains(QStringLiteral("void clearPendingSelectionRangeExport();"))
            && pageHost.contains(QStringLiteral("PageHost::navigationRejected"))
    ...
    ok &= require(
        sourceEditor.contains(QStringLiteral("qsTrId(\"qml.export_selection\")"))
