...
bool testContract(QTextStream& out)
{
    ...
    int completed = 0;
    quint64 requestId = 0;
    std::function<void(const miacode::ui::ChartDropResult&)> delayedCompletion;
    miacode::ui::ChartDropBridge bridge(
        window,
        ...
        [&completed](const miacode::ui::ChartDropResult&) { ++completed; });
    ...
    if (delayedCompletion) {
        delayedCompletion(miacode::ui::ChartDropResult{requestId, 1, true, true, false, 1, 0, {}});
    }
