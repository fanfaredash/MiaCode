...
miacode::ui::DocumentValidationProjectionInput currentInput()
{
...
bool verifyMutationPresentationState(QTextStream& err)
{
    const auto verify = [&err](const QString& mutation, bool validationAvailable) {
        ...
        const auto state = miacode::ui::projectDocumentPresentation(input);
...
bool verifyDirtyEditorKeys(QTextStream& err)
{
    ...
    const auto metadataState = miacode::ui::projectDocumentPresentation(input);
    ...
    miacode::ui::DocumentPresentationInput clean;
    clean.activeDifficultyId = 5;
    const auto cleanState = miacode::ui::projectDocumentPresentation(clean);
    ok &= require(cleanState.dirtyEditorKeys.isEmpty(),
                  QStringLiteral("a saved document marks nothing"), err);

    miacode::ui::DocumentPresentationInput many;
    many.dirty = true;
    many.metadataDirty = true;
    many.dirtyDifficultyIds = {2, 3, 4};
    many.activeDifficultyId = 2;
    const auto manyState = miacode::ui::projectDocumentPresentation(many);
...
{
    miacode::ui::DocumentSourceTransactionInput input;
    ...
    input.issues = {{2, 3, 7, miacode::ui::DocumentValidationIssueSeverity::Error,
                     QStringLiteral("bad token")}};
    const auto state = miacode::ui::projectDocumentSourceTransaction(input);
...
bool verifyMultiDifficultyStrictPreflight(QTextStream& err)
{
    ...
    const auto state = miacode::ui::projectDocumentSourceTransaction(transaction);
...
{
    miacode::AnalysisSnapshot snapshot;
    ...
    const auto current = miacode::ui::projectDocumentValidation(snapshot, 5, 42);
    ...
    for (const auto& stale : {
             miacode::ui::projectDocumentValidation(snapshot, 4, 42),
             miacode::ui::projectDocumentValidation(snapshot, 5, 43),
             [&] {
                 auto pending = snapshot;
                 pending.available = false;
                 pending.pending = true;
                 return miacode::ui::projectDocumentValidation(pending, 5, 42);
             }(),
         }) {
...
miacode::ui::DocumentValidationProjectionCache matchingCache()
{
    ...
    cache.issues = {
        {2, 3, 5, miacode::ui::DocumentValidationIssueSeverity::Error,
         QStringLiteral("bad token")},
        {4, 1, 2, miacode::ui::DocumentValidationIssueSeverity::Warning,
...
int main()
{
    ...
    const miacode::ui::DocumentValidationProjection projection =
        miacode::ui::projectDocumentValidation(currentInput(), matchingCache());
    ...
    const miacode::ui::DocumentValidationProjection staleProjection =
        miacode::ui::projectDocumentValidation(currentInput(), lateA);
    ...
    for (const miacode::ui::DocumentValidationProjectionCache mismatchedCache : {
        ...
        const miacode::ui::DocumentValidationProjection mismatchProjection =
            miacode::ui::projectDocumentValidation(currentInput(), mismatchedCache);
    ...
    const miacode::ui::DocumentValidationProjection noDifficultyProjection =
        miacode::ui::projectDocumentValidation(noDifficulty, matchingCache());
