...
#include "app/ui/editor/EditorInputBridge.h"
#include "app/ui/editor/EditorTextStyle.h"
#include "app/ui/editor/SimaiSyntaxHighlighter.h"
...
bool verifyEditorPointerRoutes(QTextStream& out, int* failed)
{
    ...
    component.setData(R"QML(
        ...
        Window {
            ...
            // Narrow stand-in for DocumentModel: only the members
    ...
    auto* pointer = window->findChild<QQuickItem*>(QStringLiteral("editorPointer"));
    if (pointer != nullptr) {
        const QPointF local(12, 12);
        const QPointF global = pointer->mapToGlobal(local);
        ...
        QMouseEvent release(QEvent::MouseButtonRelease, local, global, Qt::LeftButton,
                            Qt::NoButton, Qt::MetaModifier);
        QCoreApplication::sendEvent(pointer, &press);
        QCoreApplication::sendEvent(pointer, &release);
        QCoreApplication::processEvents();
    }
...
bool verifyReentrantImeCommitIsAppliedOnce(QTextStream& out, int* failed)
{
    ...
    miacode::ui::EditorInputBridge bridge;
    bridge.setImeInputDisabled(false);
    ...
    QObject::connect(&bridge, &miacode::ui::EditorInputBridge::imeCommitted,
...
bool verifyEditorTabsCanBeDraggedToSwap(QTextStream& out, int* failed)
{
    ...
    component.setData(R"QML(
        ...
        Window {
            ...
            QtObject {
                ...
                property int closeDifficultyCalls: 0
                property int lastCloseDifficultyId: -1
                ...
                function requestCloseDifficulty(difficultyId) {
                    closeDifficultyCalls += 1
                    lastCloseDifficultyId = difficultyId
                }
    ...
    ok &= expect(tabBar != nullptr && documentSession != nullptr,
                 QStringLiteral("editor-tab harness exposes the close guard and document stub"),
                 out, failed);
    if (tabBar != nullptr && documentSession != nullptr) {
        QMetaObject::invokeMethod(tabBar, "requestCloseTab",
                                  Q_ARG(QVariant, QVariant(QStringLiteral("difficulty:5"))));
        QCoreApplication::processEvents();
        ok &= expect(documentSession->property("closeDifficultyCalls").toInt() == 1
                         && documentSession->property("lastCloseDifficultyId").toInt() == 5
                         && state->property("openEditorTabs").toStringList().contains(QStringLiteral("difficulty:5")),
                     QStringLiteral("closing a difficulty tab asks the document session"),
                     out, failed);
        ...
        ok &= expect(!state->property("openEditorTabs").toStringList().contains(QStringLiteral("metadata"))
                         && documentSession->property("metadataDiscardCalls").toInt() == 0,
                     QStringLiteral("closing a metadata tab closes it immediately"),
                     out, failed);
    }
    return ok;
}
...
int main(int argc, char** argv)
{
    ...
    qmlRegisterType<miacode::ui::SimaiSyntaxHighlighter>("MiaCode.UI", 1, 0, "SimaiSyntaxHighlighter");
    qmlRegisterType<miacode::ui::EditorTextStyle>("MiaCode.UI", 1, 0, "EditorTextStyle");
    qmlRegisterType<miacode::ui::EditorInputBridge>(
        "MiaCode.UI", 1, 0, "EditorInputBridge");
    ...
    miacode::ui::EditorInputBridge inputBridge;
    inputBridge.setImeInputDisabled(false);
    inputBridge.setTarget(&inputTarget);
    int commits = 0;
    int compositionChanges = 0;
    bool composing = false;
    QString committedText;
    QObject::connect(&inputBridge, &miacode::ui::EditorInputBridge::imeCommitted,
                     [&commits, &committedText](const QString& value) { ++commits; committedText = value; });
    QObject::connect(&inputBridge, &miacode::ui::EditorInputBridge::imeComposingChanged,
    ...
    QFile sourceEditor(QStringLiteral("src/app/ui/editor/SourceEditor.qml"));
    ...
    expect(source.contains(QStringLiteral("processImeCommitForQml"))
               ...
               && source.contains(QStringLiteral("EditorInputBridge"))
    ...
    QFile difficultyList(QStringLiteral("src/app/ui/sidebar/DifficultyList.qml"));
    ...
    // DocumentModel emits in that order, so this pins the call rather than
    // reproducing the race.
    QFile sourceEditorFile(QStringLiteral("src/app/ui/editor/SourceEditor.qml"));
