    ...
    QString* resultStatus)
{
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        ...
        const bool firstPassOk = runProcess(
            ffmpegPath, firstPass, cancelRequested, nullptr, &error);
        const bool secondPassOk = firstPassOk && runProcess(
            ffmpegPath, secondPass, cancelRequested, nullptr, &error);
...
void PvBatchCompressionWorker::run()
{
    const bool needsFfmpeg = std::any_of(jobs_.cbegin(), jobs_.cend(), [](const PvCompressionJob& job) {
        return !job.videoPath.isEmpty() && job.originalBytes >= kPvCompressionHardLimitBytes;
    });
