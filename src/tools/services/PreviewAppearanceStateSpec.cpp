...
// ExportSession and PreviewSettingsModel — two copies of the same
...
{
    const miacode::PreviewAppearanceState state;
...
{
    miacode::PreviewAppearanceState state;
    QSignalSpy changed(&state, &miacode::PreviewAppearanceState::changed);
...
{
    bool ok = require(
        miacode::PreviewAppearanceState::variantForDirectory(QStringLiteral("skinDX"))
                == PreviewSkinVariant::Dx
            && miacode::PreviewAppearanceState::variantForDirectory(QStringLiteral("SKINDX"))
                == PreviewSkinVariant::Dx
            && miacode::PreviewAppearanceState::variantForDirectory(QStringLiteral("skinSD"))
                == PreviewSkinVariant::Standard
            && miacode::PreviewAppearanceState::variantForDirectory(QStringLiteral("myCustom"))
                == PreviewSkinVariant::Standard,
        QStringLiteral("only the DX folder maps to the DX family, case-insensitively"), err);

    miacode::PreviewAppearanceState state;
    QSignalSpy changed(&state, &miacode::PreviewAppearanceState::changed);
    QSignalSpy skinChanged(&state, &miacode::PreviewAppearanceState::skinChanged);
...
bool verifySpecificSignalsStayOnTheirOwnField(QTextStream& err)
{
    miacode::PreviewAppearanceState state;
    QSignalSpy skinChanged(&state, &miacode::PreviewAppearanceState::skinChanged);
    QSignalSpy judgeEffect(&state, &miacode::PreviewAppearanceState::judgeEffectStyleChanged);
    QSignalSpy introSound(&state, &miacode::PreviewAppearanceState::introSoundChanged);
