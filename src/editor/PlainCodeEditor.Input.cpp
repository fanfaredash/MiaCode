...
#ifdef Q_OS_MACOS
void PlainCodeEditor::mouseMoveEvent(QMouseEvent* event)
{
    QWidget* editorViewport = viewport();
    if (event == nullptr || editorViewport == nullptr) {
    ...
    const miacode::editor::DragAutoScrollStep step =
        miacode::editor::planDragAutoScrollStep(editorViewport->rect(), position);
    ...
    // Outside the viewport (the line-number gutter and the top overlay inset are
    // both viewport margins, so a leftward or upward drag lands here). Remember
    // where the platform says the pointer is, then hand the base class a clamped
    // copy: it sees an in-viewport move, stops its own QCursor-driven autoscroll
    // timer, and we drive the scrolling from these coordinates instead.
    dragAutoScrollViewportPos_ = position;
    dragAutoScrollGlobalPos_ = event->globalPosition().toPoint();
    dragAutoScrollModifiers_ = event->modifiers();
    if (!dragAutoScrollTimer_.isActive()) {
        dragAutoScrollTimer_.start(step.intervalMs, this);
    }
    forwardClampedDragMove(step.clampedPosition, event->modifiers());
    event->accept();
}
...
void PlainCodeEditor::timerEvent(QTimerEvent* event)
{
    if (event == nullptr || event->timerId() != dragAutoScrollTimer_.timerId()) {
    ...
    const miacode::editor::DragAutoScrollStep step =
        miacode::editor::planDragAutoScrollStep(editorViewport->rect(), dragAutoScrollViewportPos_);
    if (step.intervalMs == 0) {
        stopDragAutoScroll();
        return;
    }
    ...
    dragAutoScrollTimer_.start(step.intervalMs, this);
    if (QScrollBar* vbar = verticalScrollBar(); vbar != nullptr && step.verticalStep != 0) {
        vbar->triggerAction(
            step.verticalStep < 0
                ? QAbstractSlider::SliderSingleStepSub
                : QAbstractSlider::SliderSingleStepAdd);
    }
    if (QScrollBar* hbar = horizontalScrollBar(); hbar != nullptr && step.horizontalStep != 0) {
        hbar->triggerAction(
            step.horizontalStep < 0
                ? QAbstractSlider::SliderSingleStepSub
                : QAbstractSlider::SliderSingleStepAdd);
    }
    // Re-extend the selection after the scroll: the clamped point now sits over
    // the freshly revealed text, which is what "drag past the edge to keep
    // selecting" means.
    forwardClampedDragMove(step.clampedPosition, dragAutoScrollModifiers_);
}

void PlainCodeEditor::forwardClampedDragMove(const QPoint& clampedPosition, Qt::KeyboardModifiers modifiers)
{
    QWidget* editorViewport = viewport();
    if (editorViewport == nullptr) {
        return;
    ...
    const QPointF globalPosition(dragAutoScrollGlobalPos_);
    ...
    clampedEvent.setAccepted(false);
    QTextEdit::mouseMoveEvent(&clampedEvent);
}

void PlainCodeEditor::stopDragAutoScroll()
{
    if (dragAutoScrollTimer_.isActive()) {
