...
- `MIACODE_BASS_BGM_RATE_MODE`
  - Windows/macOS/Linux BASS preview BGM rate-mode override; unset defaults to pitch-preserving BASS_FX `tempo`, while `rate_transpose` / `transpose` / `source_time` / `accurate` switches to source-time-priority rate transpose for A/B listening
...
- `MIACODE_BASS_BGM_TEMPO_PRESET`
  - Windows/macOS/Linux BASS preview BGM BASS_FX window preset override; only active when `MIACODE_BASS_BGM_RATE_MODE=tempo`
...
- `MIACODE_BASS_BGM_TEMPO_PARAMS`
  - Windows/macOS/Linux BASS preview BGM custom BASS_FX window override; only active when tempo mode is active; overrides the preset with `sequence_ms,seek_ms,overlap_ms`
  - owner: `src/audio/BassPreviewAudioBackendImpl.h`, applied in `src/audio/BassPreviewAudioBackendSample.h`
- `MIACODE_PREVIEW_FRAME_PACING_DIAG`
  ...
  - `preview/quick_scene action=render_frame_profile` splits `render_submit_ms` into `resource_prep_ms` / `pass_record_ms` / `submit_tail_ms` (QRhi resource work vs render-pass recording, where a first-use pipeline is built, vs the endFrame tail); `action=render_stall_context` fires once for the first frame over 250 ms with a `dominant=` verdict; `action=layer_first_draw` timestamps the first frame each preview layer drew, i.e. the frame that compiles that layer's material and uploads its textures
...
The `latency/entry` runtime tag is an always-on, low-frequency crash breadcrumb for the user-triggered BPM/latency workflow. It records the Tools-menu or metadata-card source, every `switchToLatencyField` boundary, page entry, sandbox SFX preparation, cached-timeline capture, synthetic-chart build/parse, preview-state build, preview/timeline publication, slider range, SFX timeline configuration, and the BPM/offset auto-detection decode/envelope/algorithm stages. Each row includes current process private memory; size-bearing stages also include BPM, duration, chart/note/envelope counts, decoder backend, or result fields. Every phase is mirrored to `miacode_startup_beacon_<pid>.txt` through the synchronous Win32 beacon writer, while `MC_OP` scopes cover the entry and expensive sub-stages for the SEH op-chain shadow. This tag is forced even without `--debug` because it only emits during explicit latency actions.
