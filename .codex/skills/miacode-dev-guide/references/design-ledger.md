...
- While the preview time slider has focus, `Left` / `Right` currently move exactly one `60 FPS` frame per discrete press. Held seeking uses a dialog-owned timer instead of OS key-repeat timing, ramps linearly from `1x` toward `2x` equivalent playback speed, and the slider wheel uses that same one-frame base step.
