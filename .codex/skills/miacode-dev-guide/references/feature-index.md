...
- Shared UI compatibility helpers:
  ...
  - Owns: the still-shared theme/color source, legacy dialog localization helpers used by remaining tool dialogs, and the QWindow-native theme/chrome policy used by QML startup. The former `UiComponents` composition files were removed; dead outline/icon/style helpers and the legacy QWidget preview-slider surface are retired. QWidget-only helpers remain transitional until the product target drops `Qt6::Widgets`; playback publishes QML playhead changes through `PlaybackCoordinator::publishPreviewPlayhead`.
