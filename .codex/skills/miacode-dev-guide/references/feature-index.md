...
- QML export settings and range selection:
  ...
  - Owns: v2 single/batch export settings, difficulty reseeding, and the single-export range. `ExportRangeSelector` reads the canonical QML preview playhead, and its two handles use the same scrub lifecycle as the right-side preview transport; it never creates a second time source or changes worker task serialization.
