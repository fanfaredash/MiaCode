...
- `src/common/PreviewAudioMixConfig.h`
  ...
  - Scope: preview BASS runtime, export audio render-plan consumers, and supported-desktop-platform BASS offline mixing
...
- `src/audio/BassPreviewAudioBackendImpl.h`
  - Owns: Windows, macOS, and Linux preview BGM BASS/BASS_FX rate-mode defaults and BASS_FX tempo-window presets for pitch-preserving A/B tests
  - Current tuning note: BASS BGM defaults to pitch-preserving tempo mode with the `compact40` (`40/15/8`) window preset. `MIACODE_BASS_BGM_RATE_MODE=rate_transpose` switches to source-time-priority `BASS_ATTRIB_FREQ` mode. `MIACODE_BASS_BGM_TEMPO_PRESET` is active only in tempo mode; presets are unset/`compact40` (`40/15/8`), `stock` (plugin default), `auto` (`0/0/8`), `tight20` (`20/8/4`), `balanced30` (`30/10/6`), `smooth60` (`60/20/8`), and `wide82` (`82/28/8`). `MIACODE_BASS_BGM_TEMPO_PARAMS` overrides those presets with custom `sequence_ms,seek_ms,overlap_ms`.
  - Rule: keep local while this is a preview-only diagnostic path and export does not share live BASS_FX tempo playback
- `src/audio/PreviewAudioDeviceChangePolicy.h`
  - Owns: what counts as an output-device configuration change (sorted device-id list, default-output id) and whether that change pauses the preview
  - Scope: `PreviewAudioDeviceWatcher` and the `TimelineSection` auto-pause; not export timing, SFX scheduling, or visual clock policy
  - Rule: this policy holds NO thresholds and must not grow one. The trigger is a deterministic device event; the previous `50 ms` drift boundary was removed with the in-place re-anchor it fed. Do not add debounce/cooldown state either — the not-playing guard already absorbs Qt's per-hotplug notification burst
...
- `src/tools/media/PvCompressionPolicy.{h,cpp}`
  - Owns: shared single-file and batch-video compression size/bitrate/stream policy
  - Current tuning note: the hard limit is decimal `20,000,000` bytes and the two-pass x264 working target is `19,500,000` bytes. Output audio is removed, source frame timestamps/frame rate are preserved with `-fps_mode passthrough` and no `-r`/frame-rate filter, and an oversized first result gets one measured-size bitrate correction with a `0.98` safety ratio.
  - Rule: keep these values and ffmpeg argument contracts shared between single and batch video compression; preserve the final guard that rejects output at or above the hard limit or not smaller than the input.
...
- `src/app/quick_shell/qml/TimelineTabSurface.qml`
  ...
  - Current tuning note: the left header keeps only the zoom control, and the restored right header settings button uses a transparent `28 * headerScale` by `22 * headerScale` QML hit target normally `8 * headerScale` from the right edge, clamped past the zoom control on narrow widths. Header labels and line-start markers avoid that right-side button while zoom remains pinned left. The transparent QML hit zones forward hover/press state to the Quick header renderer so colour feedback remains visible.
