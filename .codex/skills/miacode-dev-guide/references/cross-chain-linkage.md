...
- preview/export `first` now uses the finite parsed raw `&first` value directly; blank input means zero, while unparseable or non-finite text is invalid and export reconstruction must fail instead of producing NaN markers; do not maintain a second inverted "effectiveFirst" concept
