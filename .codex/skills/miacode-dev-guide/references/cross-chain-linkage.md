...
- break-slide tails emit the ordinary `break` bucket event plus the optional `judge_break_slide` tail cheer; `judge_break_slide` can be silenced by the preview audio setting, but break slide must not grow a separate volume bucket or `break_slide_break` event chain.
