...
- QML normalization UI path: `ActivityBar` tools popup or `MainMenu` -> `QmlEditorPageHost::openNormalizeWholeChart` -> `MainView`'s window-level `NormalizeOptionsDialog` -> `MainSplitView` / `EditorPane` -> `SourceEditor::applyNormalization`. Video export rejects the route; latency and editor pages stay mounted beneath the dialog.
