...
- BASS is initialized on its default device (`BASS_Init(-1, ...)`), so Windows default-output changes are followed by BASS rather than a Qt `QMediaDevices` signal/reset path. A live BGM/device-clock divergence of at least `50 ms` queues one zero-delay `reanchorPlayingTransportAtChartSecond` transaction that pauses, seeks, and resumes BGM only. It preserves the SFX scheduler/cursor, does not pause the preview, restart timers, or touch canvas/stage-media state, and declines when BGM has naturally ended.
