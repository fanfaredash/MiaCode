...
- break-slide tails emit an ordinary `break` bucket event flagged as a break-slide tail break plus the `judge_break_slide` tail cheer; the flagged tail break can be filtered by the preview audio setting while `judge_break_slide` stays audible, but unfiltered tail breaks must remain in the same `break` event bucket as ordinary break taps and break slide must not grow a separate volume bucket or `break_slide_break` event chain.
