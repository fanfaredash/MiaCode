...
- partial export timing: when the export request is not marked as full-range, export now uses a 1.0-second preload, separate from the full-range export's 2.0-second lead-in. PV/BGM start from `timelineOriginSecond = segmentStart - preload`, but chart overlay/HUD frames are blank during the preload and begin rendering at `segmentStart`; the exported marker set is still filtered up front by `marker.second` within the simulated frame window `[timelineOriginSecond, R]`, and preview/export rendering, Muri overlays, and export SFX all consume that same filtered marker set
