...
- Preview object stats now share one `PreviewProgressStatsCache` across realtime HUD, the main-window side stats card, and export HUD rendering. Preserve both original semantics when touching it: the side stats card still treats hold and touch-hold as played at note start, while the HUD still treats hold-family rates/counts at judge time and keeps touch-hold under the touch bucket.
