...
<!-- translation-source-hash: 12e91318d95811cbd0cf8db06f132f803b8e21c75ec5b9f7087c3c2b9c9e6ef0 -->
...
- 需要确认产品逻辑、交互预期以及“哪些是硬约束、哪些还能调整”时，读 `references/design-ledger.md`。
...
2. 真正动手前先顺着下游消费者追一遍。MiaCode 里很多行为会在实时链路和导出链路中各有一份实现。
3. 如果一个行为同时存在于实时预览和导出路径里，默认把它当作同步对看的一组，除非引用文档明确说明不是。
...
- 更新完中文镜像后，运行 `python .codex/tools/check_translation_sync.py --stamp`，刷新中文文件记录的源文件哈希。
...
- 导出走 snapshot/worker 边界，序列化任务结构一旦变化，两边都要改。
