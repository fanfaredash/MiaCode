...
#else
    // D3D11 hardware frames: the NV12 decode texture is allocated at the
    // macroblock-aligned CODED size (e.g. a 300x300 H.264 frame is coded as
    // 304x304), while frame()->width/height report the DISPLAY size (300x300).
    // QtAVPlayer otherwise tells Qt the frame is the display size yet hands it the
    // larger texture, so Qt stretches UV 0..1 across the full coded texels and the
    // uninitialized padding (chroma U=V=0) samples as a pure-green border on
    // non-16-aligned videos. Report the true aligned texture size as the frame
    // size and let the viewport (below) crop back to the display region, so Qt
    // samples only the valid content. Zero-copy, no extra GPU/CPU work. (The
    // VAAPI/GL path keeps its own bufSize special case further down.)
    QSize frameFormatSize = size();
    if (handleType() == D3D11Texture2DHandle && frame()->hw_frames_ctx) {
        const auto *hwFramesCtx = reinterpret_cast<const AVHWFramesContext *>(frame()->hw_frames_ctx->data);
        if (hwFramesCtx
            && hwFramesCtx->width >= frame()->width
            && hwFramesCtx->height >= frame()->height
            && (hwFramesCtx->width > frame()->width || hwFramesCtx->height > frame()->height)) {
            frameFormatSize = QSize(hwFramesCtx->width, hwFramesCtx->height);
        }
    }

    QVideoFrameFormat videoFormat(frameFormatSize, format);
