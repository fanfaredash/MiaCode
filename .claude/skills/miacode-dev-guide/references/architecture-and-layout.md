...
| 任务 | 入口 | 复用与边界 |
| --- | --- | --- |
| 启动、窗口与装配 | `src/app/main.cpp`、`src/app/qml_ui/QmlUiBootstrap.cpp`、`src/app/runtime/` | Session 拥有宿主；QML 引擎创建根窗口 |
| 文档、打开保存、分析 | `src/app/v2/` | ChartWorkspace、ChartWorkspaceFileService、AnalysisService；不要在 QML model 复制文档权威 |
| 编辑器与书签 | `src/app/qml_ui/editor/`、`src/editor/` | QmlEditorController/InputBridge、EditorSyncController；文本策略与书签语法复用 src/editor |
| 播放、走带与同步 | `src/app/runtime/playback/`、`src/app/v2/PlaybackControl.h` | PlaybackCoordinator 是播放权威；Preview/Timeline 通过端口和带身份的快照交互 |
...
| 视频/封面导出 | `src/app/qml_ui/export/`、`src/app/runtime/export/`、`src/tools/video_export/`、`src/tools/cover_export/` | UI session、ExportEngine、snapshot/worker、合成器各负其责 |
| 延迟、无理、媒体工具 | `src/tools/latency/`、`src/tools/muri/`、`src/tools/media/` | 前端经 v2 engine 接口调用；不要复制分析实现 |
...
共享组件位于 `src/app/qml_ui/components/`，优先查看相邻页面的用法。
...
| 颜色、间距、字体与背景 | `src/app/qml_ui/theme/Theme.qml`；C++ 共享主题在 `src/app/ui/` |
| 用户可见文案 | `src/app/qml_ui/UiText.qml` 与 `src/app/ui/UiText.cpp`；沿现有本地化入口添加键 |

例如增加设置选择器：从 `src/app/qml_ui/preferences/PreferencesDialog.qml` 的
LabeledCombo 用法出发，经过 `QmlPreferencesModel` 和 PreferencesStore 接入持久化。
