...
- **Idle-freeze diagnostics:** `startup/process_identity` embeds product version plus CMake-generated
  ...
  `ui/hang_watchdog` reports both marked phases (~2 s) and idle heartbeat stalls (~5 s).
...
`MIACODE_DISABLE_STARTUP_TIMING`, `MIACODE_SKIP_ASYNCLOG_FLUSH`, `MIACODE_SKIP_CRASH_HANDLER`, `MIACODE_OPERATION_LOG_PATH`.
...
`kCompileDefinitions` in `DebugFlagIndexSpec.cpp` alongside `MIACODE_SOURCE_ROOT`.
